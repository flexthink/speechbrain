...
class VALLEBrain(sb.Brain):
    ...
    def compute_forward(self, batch, stage):
        ...
        nar_stage = torch.randint(
            1, self.hparams.audio_tokens_per_step,
            (1,),
            generator=self.nar_stage_generator
        ).item()

        logits_ar, logits_nar = self.modules.model(
            x=tokens,
            x_lens=tokens_length,
            y=audio,
            y_lens=audio_length,
            nar_stage=nar_stage,
            train_stage=self.hparams.valle_train_stage
        )
        return logits_ar, logits_nar, nar_stage
    ...
    def compute_objectives(self, predictions, batch, stage):
        ...
        logits_ar, logits_nar, nar_stage = predictions
        ...
        if logits_nar is not None:
            nar_targets = audio[:, :, nar_stage]            
        ...
        * ``compute_objectives()``
        ...
        if self.is_evaluating():
            with torch.no_grad():
                tokens, tokens_length = batch.tokens
                # Produce samples
                audio_tokens, wav_length = self.modules.model.infer(
                    x=tokens, x_lens=tokens_length
                )
                wav = self.vocoder(audio_tokens, wav_length)
                wav = wav.squeeze(1)
    ...
    def use_audio_prompt(self, stage, audio):
        if not hasattr(self, "audio_prompt"):
            self.audio_prompt = {}
        self.audio_prompt[stage] = audio.to(self.device)
    ...
    def on_stage_end(self, stage, stage_loss, epoch):
        ...
        loss_stats_ar = self.loss_metric_ar.summarize()
        loss_stats_nar = self.loss_metric_nar.summarize()
        stage_stats = {
            "loss": stage_loss,
            **{f"{key}_ar" : value for key, value in loss_stats_ar.items()},
            **{f"{key}_nar" : value for key, value in loss_stats_nar.items()},
        }
    ...
    def vocoder(self, audio, length):
        return self.modules.vocoder(audio, length)
