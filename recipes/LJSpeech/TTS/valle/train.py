...
from speechbrain.utils.data_utils import batch_pad_right, undo_padding
            ...
            with torch.no_grad():
                ...
                with self.hparams.progress_report:
                    ...
                    self.hparams.progress_report.write(
                        ids=batch.uttid,
                        audio=wav,
                        length_pred=audio_length,
                        length=batch.audio.lengths,
                        tgt_max_length=batch.audio.data.size(1),
                    )
                    # Evaluate
    ...
    def create_perfect_samples(self):
        ...
        if not self.hparams.progress_logger["perfect_samples_created"]:
            ...
            for batch in sample_loader:
                batch = batch.to(self.device)
                sample_tokens, length = batch.audio
                sample_tokens = self.select_layers(sample_tokens)
                if self.hparams.flip_layers:
                    sample_tokens = sample_tokens.flip(2)
...
def dataio_prepare(hparams):
    ...
    @sb.utils.data_pipeline.takes("audio_tokens", "tokens")
    @sb.utils.data_pipeline.provides("audio", "prefix", "prompt", "prefix_length", "length")
    def prompt_pipeline(audio, tokens):
