...
class HifiGanBrain(sb.Brain):
    def compute_forward(self, batch, stage):
        ...
        x, _ = batch.audio_tokens_segment
        y, _ = batch.sig
    ...
    def save_audio(self, name, data, epoch):
        ...
        torchaudio.save(file_name, data.cpu(), self.hparams.sample_rate)
...
def compute_hop_size(hparams):
    ...
    dummy_audio_sample_length = torch.ones(1)
    tokens, emb = token_model.encode(
        dummy_audio_sample,
        dummy_audio_sample_length
    )
    hop_size = dummy_audio_sample.size(1) // tokens.size(1)
    return hop_size
...
def dataio_prepare(hparams):
    ...
    @sb.utils.data_pipeline.provides("audio_tokens_segment", "sig")
    def audio_pipeline(wav, segment, audio_tokens):
        ...
        audio_tokens = torch.from_numpy(audio_tokens).int()

        # Maps indices from the range [0, k] to [1, k+1]
        audio_tokens = audio_tokens + 1

        # Trim end of audio
        audio_tokens_length = min(audio.shape[0] // code_hop_size, audio_tokens.shape[0])
        audio_tokens = audio_tokens[:audio_tokens_length]
        audio = audio[: audio_tokens_length * code_hop_size]

        while audio.shape[0] < segment_size:
            audio = torch.hstack([audio, audio])
            audio_tokens = torch.hstack([audio_tokens, audio_tokens])

        audio = audio.unsqueeze(0)
        if segment:
            audio_tokens = audio_tokens.unsqueeze(0)
            audio, audio_tokens = sample_interval([audio, audio_tokens], segment_size)
            audio_tokens = audio_tokens.squeeze(0)

        return audio_tokens, audio
    ...
    for dataset in hparams["splits"]:
        dynamic_dataset = sb.dataio.dataset.DynamicItemDataset.from_json(
            ...
            output_keys=["id", "audio_tokens_segment", "sig"],
        )
        add_prepared_features(
            dataset=dynamic_dataset,
            save_path=pl.Path(hparams["prepare_save_folder"]) / "features",
            id_key="uttid",
