...
def test_madmixture_basic():
    from speechbrain.lobes.models.madmixture import (
        MadMixture, Modality, AttentionalAligner)
    ...
    mock_text_enc = LengthsCapableSequential(
        mock_text_emb,
        mock_text_conv
    )
    ...
    mock_audio_align = AttentionalAligner(
        dim=16,
        scale=0.5
    )
    ...
    madmixture = MadMixture(
        modalities=[
            Modality(
                name="text",
                encoder=mock_text_enc,
                decoder=mock_text_dec
            ),
            Modality(
                name="audio",
    ...
    mock_text = torch.tensor(
        [[2, 1, 4, 0],
         [3, 1, 20, 19]]
    )
    mock_audio = torch.randn(2, 200, 1)
    mock_text_length = torch.tensor([1., 1.])
    mock_audio_length = torch.tensor([1., 1.])
