...
class UTMOSModel(nn.Module):
    ...
    def forward(self, wav, domain_id=None, judge_id=None):
        ...
        if domain_id is None:
            domain_id = torch.zeros(len(wav), dtype=torch.int, device=wav.device)
        if judge_id is None:
            judge_id = torch.ones(len(wav), dtype=torch.int, device=wav.device) * DEFAULT_JUDGE_ID
...
class UTMOSMetric(MetricStats):
    ...
    def append(self, ids, hyp_audio, lens=None, domain_ids=None, judge_ids=None):
        ...
        if domain_ids is None:
            domain_ids = torch.zeros(len(hyp_audio), dtype=torch.int, device=hyp_audio.device)
        if judge_ids is None:
            judge_ids = torch.ones(len(hyp_audio), dtype=torch.int, device=hyp_audio.device) * self.judge_id
