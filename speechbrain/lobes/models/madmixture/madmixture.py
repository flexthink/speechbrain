...
class MadMixture(nn.Module):
    ...
    def encode(self, inputs, lengths):
        ...
        return {
            key: self.modalities[key].encoder(
                input,
    ...
    def latent(self, inputs, lengths):
        ...
        """
        enc_out = self.encode(inputs, lengths)
        aligner_out = {
    ...
    def decode_single(self, latent, lengths):
        ...
        return {
            key: modality.decoder(latent, lengths=lengths)
            for key, modality in self.modalities.items()
        }

    def decode_multiple(self, latent, lengths):
        ...
        """
        return {
            key: self.modalities[key].decoder(
                modality_latent,
                length=lengths.get(key) if lengths is not None else None)
            for key, modality_latent in latent
        }
...
class AttentionalAligner(nn.Module):
    ...
    def __init__(self, dim, nhead=1, dropout=0., scale=None, kernel_size=None):
        super().__init__()
        ...
        self.scale = scale
        if scale is None or scale - 1 < 0.01:
            self.scale = 1
    ...
    def forward(self, key, enc_out, lengths):
        ...
        output, alignment = self.attn(masked_queries, mod_enc_out, mod_enc_out)
