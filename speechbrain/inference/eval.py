    ...
    def evaluate_batch(self, batch):
        ...
        with torch.no_grad():
            ...
            for evaluator_key, evaluator in self.evaluators.items():
                ...
                eval_details = undo_batch(result.details)
                self.write_result(evaluator_key, batch.uttid, eval_details)
                self.details[evaluator_key].extend(eval_details)
            if self.hparams.eval_perf and details.get("perf_stats"):
                self.write_perf_stats(batch.uttid, details["perf_stats"])
